...
class RegisterViewModel(app: Application) : BaseViewModel(app) {
    ...
    fun getRegisterResult(): LiveData<Resource<AuthResult>> {
    ...
    fun doRegister(auth: AuthData): LiveData<Resource<AuthResult>> {

        mUsername = auth.mUsername
        mPassword = auth.mPassword

        val rxFlow = mFbRepos.handleRegisterByEmail(mUsername, mPassword)
